...
python src/validate_graph.py \
  --graph data/DB100k+/reconstructed-datasets/train+val+schema_dataset.ttl \
  --shapes data/DB100k+/cleaned-shapes/cleaned_shapes.ttl \
  --output-safe-iris data/DB100k+/validation-reports/validation_report_train+val+schema_dataset_safe_iris.ttl \
  --output-original-iris data/DB100k+/validation-reports/validation_report_train+val+schema_dataset_original_iris.ttl
...
python src/validate_graph.py \
  --graph data/NELL-995+/reconstructed-datasets/train+val+schema_dataset.ttl \
  --shapes data/NELL-995+/cleaned-shapes/cleaned_shapes.ttl \
  --output-safe-iris data/NELL-995+/validation-reports/validation_report_train+val+schema_dataset_safe_iris.ttl \
  --output-original-iris data/NELL-995+/validation-reports/validation_report_train+val+schema_dataset_original_iris.ttl
...
python src/validate_graph.py \
  --graph data/YAGO3-10+/reconstructed-datasets/train+val+schema_dataset.ttl \
  --shapes data/YAGO3-10+/cleaned-shapes/cleaned_shapes.ttl \
  --output-safe-iris data/YAGO3-10+/validation-reports/validation_report_train+val+schema_dataset_safe_iris.ttl \
  --output-original-iris data/YAGO3-10+/validation-reports/validation_report_train+val+schema_dataset_original_iris.ttl
